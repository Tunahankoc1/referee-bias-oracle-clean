  ...
  try {
    ...
    data.fixtures.slice(0, 10).forEach(m => {
      html += `<div style="padding:8px; border-bottom:1px solid #2a4a36;">
        <strong>${m.home_team} vs ${m.away_team}</strong> - ${m.status}
      </div>`;
    });
...
function calc() {
  ...
  document.getElementById('breakdown').innerHTML = 
    `<div>Card Score: ${cardScore.toFixed(1)} (×0.6)</div>
     <div>Penalty Score: ${penaltyScore.toFixed(1)} (×0.4)</div>
     <div style="margin-top:5px; color:#d4a843;"><strong>Final: ${s}</strong></div>`;
...
function submit() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      log('Score: ' + s);
      log('<a style="color:#d4a843;" href="https://explorer.solana.com/tx/' + tx + '?cluster=devnet" target="_blank">' + tx + '</a>');
    }, 1500);
  }, 1000);
}
...
def get_fixtures():
    try:
        ...
        if not jwt_token:
            return jsonify({'error': 'Failed to get JWT'}), 500
